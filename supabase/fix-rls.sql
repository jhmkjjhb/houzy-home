...
-- pending_confirm 状态迁移（新数据库执行 schema.sql 已包含，旧库执行以下）
...
-- ));
-- confirm_order_by_phone 函数已在 schema.sql 中定义，请在 SQL Editor 中执行。
-- Storage bucket: 在 Supabase Dashboard → Storage 创建公开 bucket "order-images"
